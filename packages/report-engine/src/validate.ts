import { WeeklyReport, ValidationResult } from './types';

export function validateReport(report: WeeklyReport): ValidationResult {
  const issues: string[] = [];

  // Required metadata
  const requiredMeta = ['organization', 'team', 'weekStart', 'weekEnd', 'author', 'rag', 'sensitivity'];
  for (const field of requiredMeta) {
    if (!report.metadata[field as keyof typeof report.metadata]) {
