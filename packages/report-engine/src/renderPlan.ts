import { WeeklyReport, RenderPlan, AnchorMapping } from './types';
import { computeWeekLabel, computeSummary } from './compute';

export function generateRenderPlan(report: WeeklyReport): RenderPlan {
  const anchors: AnchorMapping = {
    META: `**Week:** ${computeWeekLabel(report)}\n**Team:** ${report.metadata.team}\n**Author:** ${report.metadata.author}\n**RAG:** ${report.metadata.rag}`,
